...
- Commands, process checks, and file diffs use the shared `tool-output-header` layout for matching height, padding, typography, and vertically centered text/actions. Their body separators span the full card width; diff rows and change markers reach the card edges.
